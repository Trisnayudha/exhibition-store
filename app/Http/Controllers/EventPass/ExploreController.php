...
class ExploreController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $file_name = 'Explore_Pass_Excel_' . time() . '.xlsx';
        ...
        $log = ExhibitionLog::where('section', 'explore')->where('company_id', auth()->id())->first();
        if ($log == null) {
...
Ada yang submit Explore Pass nih
...
';
    ...
    public function destroy($id)
    {
        ...
        $company_id = auth()->id();
        $log = ExhibitionLog::where('section', 'explore')->where('company_id', $company_id)->first();
        if ($log == null) {
            $log = new ExhibitionLog();
            $log->section = 'explore';
    ...
    {
        $userId = auth()->id();
        $data = ExhibitionLog::where('section', 'explore')->where('company_id', $userId)->first();
        return $data;
    }
}
